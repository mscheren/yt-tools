...
def _create_progress_callback() -> callable:
    """Create a progress callback that prints download progress."""
...
def cli():
    """YouTube downloader with support for videos and playlists."""
    pass
...
def video(url: str, output_dir: Path, output_format: str, cookies: Path | None):
    ...
    try:
        ...
        click.echo(f"Successfully downloaded: {title}")
...
def playlist(url: str, output_dir: Path, output_format: str, cookies: Path | None):
    ...
    try:
        ...
        click.echo(f"Successfully downloaded playlist: {title} ({len(entries)} items)")
    except Exception as e:
        raise click.ClickException(f"Download failed: {e}") from e
...
def info(url: str):
    ...
    try:
        ...
        if "entries" in metadata:
            ...
            click.echo(f"Type: Single Video")
